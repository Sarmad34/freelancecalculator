    ...
    <div className="max-w-7xl mx-auto px-4 py-16 md:py-24">
      <div className="max-w-4xl mb-20">
        <h1 className="text-5xl md:text-7xl font-black text-slate-900 mb-8 tracking-tight leading-[1.1]">
          The Definitive <span className="text-blue-600 underline decoration-blue-100 underline-offset-8">Freelance Financial</span> Suite
        </h1>
        <p className="text-xl md:text-2xl text-slate-600 mb-10 font-medium leading-relaxed max-w-2xl">
          High-trust pricing tools for independent contractors. Use real-world business math to set rates, audit overhead, and hit your 2025 revenue goals.
        </p>
      ...
      <div className="prose prose-slate max-w-4xl mx-auto py-16 border-t border-slate-100">
        ...
        {content.intro.map((p, i) => <p key={i} className="text-xl text-slate-600 leading-relaxed">{p}</p>)}
        
        {content.sections.map((section, idx) => (
          <div key={idx} className="mt-16">
            <h3 className="text-3xl font-bold text-slate-900 tracking-tight mb-6">{section.h2}</h3>
            {section.body.map((p, i) => <p key={i} className="text-lg">{p}</p>)}
            {section.bullets && (
              <ul className="grid grid-cols-1 sm:grid-cols-2 gap-x-12 gap-y-6 mt-10 list-none pl-0">
                {section.bullets.map((b, i) => (
                  <li key={i} className="flex gap-4 text-slate-600 font-semibold items-start text-lg">
                    <span className="w-8 h-8 bg-blue-100 text-blue-600 rounded-xl flex items-center justify-center flex-shrink-0 text-sm font-black shadow-sm">✓</span>
                    <span>{b}</span>
                  </li>
                ))}
              </ul>
        ...
        <div className="bg-slate-900 rounded-[2.5rem] p-12 mt-24 text-white not-prose">
          <h3 className="text-3xl font-black mb-10 tracking-tight">Expert Strategy & FAQ</h3>
          <div className="space-y-10">
            {content.faqs.map((faq, i) => (
              <div key={i} className="group border-b border-slate-800 pb-8 last:border-0">
                <h4 className="text-xl font-bold text-blue-400 mb-4">{faq.q}</h4>
      ...
      <div className="mt-24 pt-16 border-t border-slate-100">
        <h2 className="text-2xl font-bold mb-10 text-center tracking-tight text-slate-900 uppercase">Role-Specific Market Data</h2>
        <div className="flex flex-wrap justify-center gap-3">
          {FREELANCE_ROLES.map(role => (
            <Link 
              key={role} 
              to={`/hourly-rate-calculator/${role.toLowerCase().replace(/ /g, '-')}/`}
              className="px-6 py-3 bg-white border border-slate-200 rounded-2xl text-sm font-bold text-slate-600 hover:border-blue-500 hover:text-blue-600 hover:bg-blue-50 transition-all shadow-sm active:scale-95"
            >
              {role}
            </Link>
          ))}
